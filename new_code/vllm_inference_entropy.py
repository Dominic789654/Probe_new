...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Run vLLM inference for pass@k and calculate entropy.")
...
def main():
    ...
    print(f"Prepared {len(templates)} templates for generation.")
    ...
    with open(args.output_file_path, 'w', encoding='utf-8') as f:
        ...
        for i in tqdm(range(0, len(templates), args.batch_size), desc="Processing Batches"):
            batch_templates = templates[i:i+args.batch_size]
            batch_original_data = original_data[i:i+args.batch_size]

            # Generate outputs for the current batch
            batch_outputs = llm.generate(batch_templates, sampling_params)
