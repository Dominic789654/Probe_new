...
class VLLMMathProblemSolver:
    ...
    def batch_generate_responses(self, questions: List[str], strategies: List[str], 
                                 ...
                                 continuation_max_tokens: int = 128) -> List[Tuple[str, Dict[str, int]]]:
        ...
        hard_params = SamplingParams(n=n_runs, temperature=temperature*2, min_p=0.2, max_tokens=max_tokens // 2, logprobs=10, repetition_penalty=1.1)
                ...
                if generated_sample.finish_reason == 'length':
                    ...
                    if len(prompt_token_ids) >= max_model_len:
                        ...
                        if new_max_len <= 0:
                            ...
                            token_stats = {'total_generated_tokens': len(generated_sample.token_ids)}
                            final_results[result_key] = (response_text, token_stats)
        ...
        if continuation_prompts:
            ...
            for i, second_gen_output in enumerate(continuation_outputs):
                ...
                total_tokens = len(first_sample.token_ids) + len(self.tokenizer.encode(continuation_prompt, add_special_tokens=False)) + len(second_sample.token_ids)
                token_stats = {'total_generated_tokens': total_tokens}
                final_results[result_key] = (final_response, token_stats)
...
def run_generation_stage(args, probe_results):
    ...
    for i in tqdm(range(0, len(probe_results), args.batch_size)):
        ...
        for j, responses in enumerate(batch_responses_list):
            ...
            for run_idx, (response, token_stats) in enumerate(responses):
                result_item = {
                    **probe_res, 
                    'response': response, 
                    'run_id': run_idx,
...
def main():
    ...
    if not args.baseline_mode and not args.full_baseline_mode and not args.tale_ep_baseline and not args.concise_cot_baseline and not args.random_baseline_mode and not args.probe_path:
